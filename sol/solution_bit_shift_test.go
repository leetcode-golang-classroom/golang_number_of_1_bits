...
func BenchmarkTestV2(b *testing.B) {
	var num uint32 = 0b11111111111111111111111111111101
	for idx := 0; idx < b.N; idx++ {
		hammingWeightV2(num)
	}
}
func Test_hammingWeightV2(t *testing.T) {
	type args struct {
		num uint32
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := hammingWeightV2(tt.args.num); got != tt.want {
				t.Errorf("hammingWeightV2() = %v, want %v", got, tt.want)
			}
		})
	}
}
